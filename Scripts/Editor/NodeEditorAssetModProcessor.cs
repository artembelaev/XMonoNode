...
namespace XMonoNodeEditor {
    ...
    class NodeEditorAssetModProcessor : UnityEditor.AssetModificationProcessor {
        ...
        private static AssetDeleteResult OnWillDeleteAsset (string path, RemoveAssetOptions options) {
            ...
            if (scriptType == null || (scriptType != typeof (XMonoNode.Node) && !scriptType.IsSubclassOf (typeof (XMonoNode.Node)))) return AssetDeleteResult.DidNotDelete;
            ...
            for (int i = 0; i < guids.Length; i++) {
                ...
                for (int k = 0; k < objs.Length; k++) {
                    XMonoNode.Node node = objs[k] as XMonoNode.Node;
        ...
        private static void OnReloadEditor () {
            ...
            string[] guids = AssetDatabase.FindAssets ("t:" + typeof (XMonoNode.NodeGraph));
            for (int i = 0; i < guids.Length; i++) {
                string assetpath = AssetDatabase.GUIDToAssetPath (guids[i]);
                XMonoNode.NodeGraph graph = AssetDatabase.LoadAssetAtPath (assetpath, typeof (XMonoNode.NodeGraph)) as XMonoNode.NodeGraph;
                ...
                for (int u = 0; u < objs.Length; u++) {
                    ...
                    if (!graph.nodes.Contains (objs[u] as XMonoNode.Node)) graph.nodes.Add(objs[u] as XMonoNode.Node);
                }
            }
        }
    }
